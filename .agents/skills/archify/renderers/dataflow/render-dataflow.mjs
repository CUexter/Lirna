...
function validateDataflow() {
  const problems = [];
  if (dataflow.schema_version !== 1)
    problems.push('Data-flow files must set "schema_version": 1.');
  if (dataflow.diagram_type !== "dataflow")
    problems.push('Data-flow files must set "diagram_type": "dataflow".');
  if (!dataflow.meta?.title)
    problems.push("Data-flow files must include meta.title.");
