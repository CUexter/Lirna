...
async function commandVisualCheck(args) {
  ...
  if (json) {
    ...
    console.log(
      `visual-check ${result.receipt.status}: ${result.receipt.artifact.path}`,
    );
    console.log(
...
function commandValidate(args) {
  ...
  if (layoutJson) {
    if (type !== "architecture") {
      fail(
        "--layout-json is currently supported for architecture diagrams only.",
      );
    }
