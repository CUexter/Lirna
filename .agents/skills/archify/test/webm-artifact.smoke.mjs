...
try {
  ...
  async function verifyArchitectureDeltaNavigator(file) {
    ...
    async function waitForSelected(changeKey, label) {
      ...
      assert.fail(
        `${label} did not select ${changeKey} within the bounded wait`,
      );
    }
  ...
  async function captureShareCard(file, label) {
    ...
    const largestColorShare =
      Math.max(...counts.values()) / (pixels.length / 3);
    assert.ok(
      colors.size >= 24,
  ...
  async function captureCopiedShareCard(file, label) {
    ...
    assert.equal(
      png.readUInt32BE(16),
      1200,
      `${label} copied Share Card width`,
    );
    assert.equal(
      png.readUInt32BE(20),
      630,
      `${label} copied Share Card height`,
    );
  ...
  async function captureRouteShareCard(
    ...
    assert.deepEqual(
      routePayload.matchedNodeIds,
      routePayload.snapshot.nodeIds,
    );
    assert.deepEqual(
