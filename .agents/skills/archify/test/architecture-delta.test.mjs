...
test("compare CLI writes a deterministic three-state artifact and complete sidecar receipt", () => {
  ...
  assert.match(
    firstHtml,
    /data-node-id="cache"[^>]+data-delta-state="removed"/,
  );
  ...
  assert.match(
    firstHtml,
    /data-change-key="boundary:region:Production region"/,
  );
...
test("formatting-only input changes raw proof but not semantic hash or artifact bytes", () => {
  ...
  assert.notEqual(
    originalReceipt.base.rawSha256,
    changedReceipt.base.rawSha256,
  );
  assert.equal(
