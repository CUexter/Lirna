...
function workflowEdgePoints(html, id) {
  ...
  assert.ok(
    pointsAttribute,
    `expected rendered workflow edge points for ${id}`,
  );
...
test("architecture: boundary labels and their masks paint above relationship routes", () => {
  ...
  assert.ok(
    lastRoute >= 0,
    "expected at least one rendered relationship route",
  );
  assert.ok(
...
test("sequence: segment titles render as foreground badges above their borders", () => {
  ...
  assert.match(
    html,
    new RegExp(`data-graph-role="segment-label"[^>]*data-segment-id="0"`),
  );
  assert.match(
    html,
    new RegExp(
