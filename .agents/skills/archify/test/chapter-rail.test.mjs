...
test("chapter rail delegates selection and mirrors the existing activeIndex owner", () => {
  ...
  assert.doesNotMatch(
    html,
    /selectedChapter|visitedChapters|completedChapters/,
  );
});
...
test("chapter rail has positional, touch, mobile, motion, embed, and print boundaries", () => {
  ...
  assert.match(
    html,
    /data-chapter-position="before"[\s\S]*border-style: solid/,
  );
  assert.match(
    html,
    /data-chapter-position="after"[\s\S]*border-style: dashed/,
  );
