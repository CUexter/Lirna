...
that evidence with the manifest and lockfile change. The automated confidence
score is bound to the exact package version and tarball. Scores below the policy
threshold produce visible findings but do not stop installation or require an
override; inability to establish artifact identity or integrity still fails
closed. Direct `npm install`, `uninstall`, `update`, `link`, `npx`, and `npm exec`
are not supported Coding-agent paths. `npm search`, `npm view`, repository
scripts, and `npm ci --ignore-scripts` remain available. The pre-commit hook and
CI detect direct dependency additions and verify the recorded score, but are
detection controls rather than a claim to block every external process.
