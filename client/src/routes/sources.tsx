  ...
  return (
    <main className="mx-auto w-[min(48rem,calc(100%-2rem))] py-12 sm:py-20">
      ...
      <form onSubmit={submit} className="mt-10 grid gap-6 border-t border-border pt-8">
        ...
        <label className="text-sm font-semibold" htmlFor="publication-text">
          Publication text
          <Textarea className="mt-2" id="publication-text" name="text" required />
        </label>
        <div className="grid gap-6 sm:grid-cols-2">
          <label className="text-sm font-semibold">
            ...
            <select className={fieldClass} name="rightsBasis" defaultValue="" required>
              ...
              {rightsBasisOptions.map(([value, label]) => (
          ...
          <label className="text-sm font-semibold">
            ...
            <select
              ...
              defaultValue="ordinary-cloud"
              required
            >
              {sensitivityLevelOptions.map(([value, label]) => (
                <option key={value} value={value}>
                  {label}
                </option>
              ))}
        ...
        <div>
          <Button type="submit" disabled={admission.isPending}>
            Admit Source
          </Button>
          {admission.error ? (
            <p className="mt-3 text-sm text-destructive">{admission.error.message}</p>
          ) : null}
        </div>
      </form>
    </main>
...
export function SourceReaderRoute({ sourceId }: { sourceId: string }) {
  ...
  const source = useQuery({
    ...
    queryFn: () => readSource(sourceId, accessToken),
  });
  ...
  return (
    <main className="mx-auto w-[min(56rem,calc(100%-2rem))] py-12 sm:py-20">
      ...
      <article
        ...
        className="mt-10 max-w-[68ch] whitespace-pre-wrap font-mono text-base leading-7"
      >
      ...
      <aside className="mt-12 border-t border-border pt-6">
        ...
        {evidence.data ? (
          <pre
            data-authoritative-evidence
            className="mt-4 overflow-x-auto whitespace-pre-wrap rounded-md border border-border bg-card p-5 text-sm"
          >
            {evidence.data}
          </pre>
        ) : null}
