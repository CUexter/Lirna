...
async function main() {
  ...
  if (
    ...
    throw new Error(
      "usage: npm run dependency:run-scripts -- <exact-package@version>",
    );
  }
  ...
  if (lock.packages?.[`node_modules/${name}`]?.version !== version) {
    throw new Error(
      `lockfile does not contain exact package ${name}@${version}`,
    );
  }
  ...
  const flake =
    process.env.LIRNA_NIX_FLAKE ??
    resolve(new URL("..", import.meta.url).pathname);
  const disposableRoot = await mkdtemp(
    join(tmpdir(), "lirna-dependency-scripts-"),
  );
  try {
    await cp(
      join(projectRoot, "package.json"),
      join(disposableRoot, "package.json"),
    );
    await cp(
      join(projectRoot, "package-lock.json"),
      join(disposableRoot, "package-lock.json"),
    );
    await exec(
      nix,
      [
  ...
  console.log(
    `Ran required scripts for ${name}@${version} in the disposable Nix environment.`,
  );
}
