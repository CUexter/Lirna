  ...
  for (const dependency of additions) {
    ...
    for (const override of assessment.requiredOverrides ?? []) {
      const record = await readJson(
        revisions.target,
        `config/dependency-decisions/${decisionIdentity(dependency.name, dependency.version)}.${override.kind}.json`,
      );
      if (!record) {
        throw new Error(
      ...
      await validateOverride(record, override, dependency, revisions.target);
    }
  }
  console.log(
...
async function validateOverride(record, override, dependency, revision) {
  if (override.kind === "warnings") {
    validateOfficialSourceUrl(record.officialSourceUrl);
    if (!sameStrings(record.triggeredWarnings, override.triggeredWarnings)) {
      throw new Error(
  ...
  if (override.kind === "critical-vulnerabilities") {
    if (!sameStrings(record.vulnerabilityIds, override.vulnerabilityIds)) {
      throw new Error(
        `critical exception for ${dependency.name}@${dependency.version} does not match assessment evidence`,
      );
    }
    const author = await decisionAuthor(revision, dependency, override.kind);
    if (
