...
export async function readCommittedDecision(projectRoot, path) {
  ...
  try {
    await exec("git", ["diff", "--quiet", "HEAD", "--", relativePath], { cwd: projectRoot });
    await exec("git", ["ls-files", "--error-unmatch", relativePath], { cwd: projectRoot });
  } catch {
    throw new Error(`dependency decision must be committed and unchanged: ${relativePath}`);
  }
  return JSON.parse(await readFile(path, "utf8"));
}

export function validateExactDecision(record, { name, version, date, maximumAgeDays }) {
  if (record.package !== name || record.version !== version) {
    throw new Error(`dependency decision does not match exact package ${name}@${version}`);
  }
  ...
  const age = (new Date(date).getTime() - new Date(record.assessmentDate).getTime()) / 86_400_000;
...
export async function lastCommitAuthor(projectRoot, path) {
  const relativePath = path.slice(projectRoot.length + 1);
  const { stdout } = await exec("git", ["log", "-1", "--format=%ae", "--", relativePath], {
    cwd: projectRoot,
  });
