...
export function classifyAssessment(findings, policy) {
  if (!Number.isInteger(policy.policyVersion) || policy.policyVersion < 1) {
    throw new Error("dependency assessment policy requires a positive policyVersion");
  }
  if (
    ...
    throw new Error("dependency assessment policy has invalid confidence thresholds");
  }
  if (!Array.isArray(findings)) throw new Error("assessment findings must be an array");
  ...
  const confidenceScore = Math.max(
    0,
    100 - scoredFindings.reduce((total, finding) => total + finding.deduction, 0),
  );
...
export function validateAssessmentClassification(record, policy) {
  ...
  if (
    ...
    throw new Error("recorded confidence classification does not match assessment policy");
  }
}
