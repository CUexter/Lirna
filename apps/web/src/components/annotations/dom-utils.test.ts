...
test("places annotation callouts outside the reading article", () => {
  expect(calloutPosition({ left: 100, right: 700 }, 1000)).toEqual({
    left: 716,
    side: "right",
  });
  expect(calloutPosition({ left: 300, right: 900 }, 1000)).toEqual({
    left: 60,
    side: "left",
  });
  expect(calloutPosition({ left: 100, right: 900 }, 1000)).toBeUndefined();
});
