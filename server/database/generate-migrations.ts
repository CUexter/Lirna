...
try {
  ...
  await new Promise<void>((resolve, reject) => {
    const child = spawn(join(root, "node_modules/.bin/drizzle-kit"), ["generate", "--config", config], { stdio: "inherit" });
    child.once("exit", (code) => code === 0 ? resolve() : reject(new Error(`drizzle-kit exited ${code}`)));
  });
  console.log("Review generated SQL and preserve custom trigger statements from the committed baseline.");
} finally {
