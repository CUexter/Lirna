...
async function reviseRecord(c: Context, dependencies: ApiDependencies): Promise<Response> {
  ...
  if (
    typeof moduleName !== "string" || !moduleNamePattern.test(moduleName) ||
    typeof label !== "string" || label.length > 200 ||
    typeof note !== "string" || note.length === 0 || note.length > 200 ||
    typeof payload !== "object" || payload === null || Array.isArray(payload)
  ) return c.json({ error: "Invalid revision request" }, 400);

  try {
    const view = await dependencies.domain.module(moduleName).revise({
      recordId, label, note, payload: payload as Record<string, unknown>,
    });
    return c.json(view, 200);
  } catch (error) {
