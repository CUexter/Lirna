...
export function researchInstructions(): string[] {
  return [
    ...
    "Use groundEvidence with a natural-language intent and bounded componentScope for every passage that may materially ground the answer. It finds and admits a uniquely best passage in one call. componentScope accepts only exact identity values from the Source component records; never substitute labels or guess identity formats, and never send quotation text, offsets, occurrence numbers, prefixes, or suffixes.",
    "If groundEvidence reports ambiguous candidates, select one by calling admitEvidence with only its opaque candidateHandle.",
    "A successful groundEvidence or admitEvidence call returns an evidence alias such as ev_1; use only successfully admitted aliases in the final answer.",
    ...
    "Answer narrow follow-up requests directly. If the latest user asks only for a direct quotation, provide at most a brief attribution and the empty quote block; do not recap the previous answer.",
