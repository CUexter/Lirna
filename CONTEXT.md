...
**Source component**:
An addressable part of a Source-state bundle, such as a supplement or notes document, that belongs to the same citable publication while retaining its own title, content, and anchors.
_Avoid_: separate Source, attachment
...
**Admission preview**:
A temporary, bounded capture of a candidate publication and its required components that exposes identity, policy, completeness, and cost before Nathan explicitly admits those exact bytes as a Source state.
_Avoid_: Source, draft Source

**Bibliography entry**:
A publisher-authored item in one Source component's scholarly apparatus. It remains part of that Source state and does not become a Reference or Source unless Nathan separately uses or admits the cited publication.
_Avoid_: Reference, cited Source
